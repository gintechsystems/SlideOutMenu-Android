...
import com.gintechsystems.slideoutmenu.SlideOutMenu.CanvasTransformer;
...
public class CustomViewBehind extends ViewGroup {
    ...
    private int mTouchMode = SlideOutMenu.TOUCHMODE_MARGIN;
    ...
    public void setMode(int mode) {
        if (mode == SlideOutMenu.LEFT || mode == SlideOutMenu.RIGHT) {
            if (mContent != null)
    ...
    public int getMenuPage(int page) {
        ...
        if (mMode == SlideOutMenu.LEFT && page > 1) {
            return 0;
        } else if (mMode == SlideOutMenu.RIGHT && page < 1) {
            return 2;
        } else {
            return page;
        }
    ...
    public int getAbsLeftBound(View content) {
        if (mMode == SlideOutMenu.LEFT || mMode == SlideOutMenu.LEFT_RIGHT) {
            return content.getLeft() - getBehindWidth();
        } else if (mMode == SlideOutMenu.RIGHT) {
            return content.getLeft();
        }
    ...
    public int getAbsRightBound(View content) {
        if (mMode == SlideOutMenu.LEFT) {
            return content.getLeft();
        } else if (mMode == SlideOutMenu.RIGHT || mMode == SlideOutMenu.LEFT_RIGHT) {
            return content.getLeft() + getBehindWidth();
        }
        return 0;
    }
    ...
    public boolean menuOpenTouchAllowed(View content, int currPage, float x) {
        switch (mTouchMode) {
            case SlideOutMenu.TOUCHMODE_FULLSCREEN:
                return true;
            case SlideOutMenu.TOUCHMODE_MARGIN:
    ...
    public boolean menuTouchInQuickReturn(View content, int currPage, float x) {
        if (mMode == SlideOutMenu.LEFT || (mMode == SlideOutMenu.LEFT_RIGHT && currPage == 0)) {
            return x >= content.getLeft() - mMarginThreshold;
        }
        else if (mMode == SlideOutMenu.RIGHT || (mMode == SlideOutMenu.LEFT_RIGHT && currPage == 2)) {
